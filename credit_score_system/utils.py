...
class DataLoader:
    ...
    def load_monthly_data(self, year: int, month: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        try:
            # 记录开始加载
            logger.info(f"开始加载 {year}年{month}月 数据")
            ...
            # 记录SQL查询
            logger.debug(f"销售数据查询SQL: {queries['sales']}")
            ...
            with self.db_manager.get_connection() as conn:
                ...
                # 记录数据获取情况
                logger.info(f"获取到销售记录 {len(sales_df)} 条")
                logger.info(f"获取到客户记录 {len(collection_df)} 条")
                logger.info(f"获取到合同记录 {len(contract_df)} 条")

                # 如果数据为空，记录警告
                if sales_df.empty:
                    logger.warning("未获取到销售数据")
